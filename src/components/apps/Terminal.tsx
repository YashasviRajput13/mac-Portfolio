...
export default class Terminal extends React.Component<{}, TerminalState> {
  ...
  help = () => {
    const help = (
      <ul className="list-disc ml-6 pb-1.5">
        ...
        <li>
          <span text-red-400>back</span> - Go back to the parent directory
        </li>
        <li>
  ...
  generateInputRow = (id: number) => {
    const newRow = (
      <div key={`terminal-input-row-${id}`} className="flex items-center space-x-1.5">
        <span text-yellow-200 className="whitespace-nowrap">
          yashasvi@macbook-pro <span text-green-300>{this.getCurDirName()}</span>
        </span>
  ...
  render() {
    return (
      <div
        className="terminal font-terminal font-normal relative h-full bg-gray-800/90 overflow-y-scroll flex flex-col text-white text-sm"
      ...
      >
        ...
        <div id="terminal-content" className="px-1.5 pb-2 flex-1">
          {this.state.content}
        </div>
      </div>
    );
